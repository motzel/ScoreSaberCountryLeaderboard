...
{#if song}
    ...
    {#if song.percent} <div><span class="scoreBottom">{$_.songBrowser.fields.acc}: <Value value="{song.percent*100}" withZeroSuffix={true} prevValue={song.prevPercent*100} inline={true} suffix={'%' + (song.mods && song.mods.length ? ' (' + song.mods + ')' : '')} /></span></div> {/if}
